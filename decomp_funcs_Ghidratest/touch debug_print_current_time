...

{
  ...
  if ((pc->dates_seen == 0) || (pc->debug_dates_seen != false)) {
    n = 0;
    if (pc->year_seen != pc->debug_year_seen) goto LAB_00105738;
  }
  else {
...
LAB_00105738:
  ...
  if ((pc->times_seen == 0) || (pc->debug_times_seen != false)) {
    if ((pc->days_seen != 0) && (pc->debug_days_seen == false)) {
      if ((char)n != '\0') goto LAB_001059f7;
      goto LAB_001057ac;
    }
  }
  else {
    __fprintf_chk(stderr,1,(n ^ 1) + 0x112af6,pc->hour,pc->minutes,(pc->seconds).tv_sec);
    if ((pc->seconds).tv_nsec != 0) {
      __fprintf_chk(stderr,1,".%09d");
    }
    if (pc->meridian == 1) {
      fwrite(&DAT_00112b0f,1,2,stderr);
    }
...
LAB_001057ac:
  ...
  if ((pc->local_zones_seen == 0) || (pc->debug_local_zones_seen != false)) {
    ...
    if (pc->timespec_seen == false) goto LAB_001058f4;
    _Var7 = (pc->seconds).tv_sec;
    if ((char)n != '\0') goto LAB_00105886;
  }
  else {
    pcVar4 = " DST";
    if (pc->dsts_seen == 0) {
    ...
    __fprintf_chk(stderr,1,(n ^ 1) + 0x112b12,pc->local_isdst,pcVar4);
